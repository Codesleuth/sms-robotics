...
function SmsRouter(players) {
  ...
  if (!esendex_push) {
    log.error('Unable to initialise SmsRouter - ESENDEX_PUSH env variable is not defined. ' +
      'SMS will not be available for control.');
    return;
  }
...
SmsRouter.prototype.sendSMS = function (recipient, body, cb) {
  if (!esendex_config.accountreference) {
    log.error('Unable to send SMS, account reference not set.');
    return;
  }
  ...
  try {
    this.esendex.messages.send(message, function (err, response) {
      if (err) log.error('Unable to send SMS to %s: ', recipient, err);
      if (isFunction(cb)) cb(err);
    });
  } catch (ex) {
    log.error('Error sending SMS to %s: ', recipient, ex);
  }
}
...
SmsRouter.prototype.handleLeave = function (player, originator) {
  ...
  this.sendSMS(originator,
    "Alright, you've been removed! Thanks for playing!",
    function (err) {
      if (!err) log.info('Courtesy "leave" SMS sent to %s.', originator);
    });
};
