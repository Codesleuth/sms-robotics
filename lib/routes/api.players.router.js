...
function EncodePlayer(player, score) {
  ...
  result.links = [{
    ...
    rel: "left",
    href: "/api/players/" + id + "/left"
  },{
    rel: "right",
    href: "/api/players/" + id + "/right"
  },{
    rel: "up",
    href: "/api/players/" + id + "/up"
  },{
    rel: "down",
    href: "/api/players/" + id + "/down"
  },{
    rel: "bomb",
    href: "/api/players/" + id + "/bomb"
  }];
...
module.exports = function (players, leaderboard) {
  ...
  router.get('/', function (req, res) {
    var leaderScores = leaderboard.getScores();

    var respondedIds = [];

    var responseArray = players.list().map(function (player) {
      var score = leaderboard.getPlayerScore(player.getId(), player.getName());
      respondedIds.push(player.getId());
      return EncodePlayer(player, score);
    });


    var leaderIds = Object.keys(leaderScores);
    var filteredIds = leaderIds.filter(function (leaderId) {
      return respondedIds.indexOf(leaderId) === -1;
    });

    filteredIds.forEach(function (leaderId) {
      var leaderPlayer = leaderScores[leaderId];
      responseArray.push({
        id: leaderId,
        name: leaderPlayer.name,
        kills: 0,
        deaths: 0,
        total_kills: leaderPlayer.kills,
        total_deaths: leaderPlayer.deaths,
        dead: true
      });
    });
  ...
  router.post('/', bodyParser.json(), function (req, res) {
    ...
    var score = leaderboard.getPlayerScore(player.getId(), req.body.name);
